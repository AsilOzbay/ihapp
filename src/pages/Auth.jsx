...
const Auth = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isLogin) {
      try {
        ...
        if (res.data.message && res.data.message.includes('2FA code sent')) {
  ...
  const handleVerify2FA = async () => {
    try {
      const res = await axios.post('http://localhost:5000/verify-2fa', {
        userId: userIdFor2FA,
        code: twoFACode,
      });
      if (res.data.token && res.data.user) {
        localStorage.setItem('token', res.data.token);
        localStorage.setItem('user', JSON.stringify(res.data.user));
        alert(`2FA successful! Welcome, ${res.data.user.firstName} ${res.data.user.lastName}`);
        if (from === "/portfolio") {
          navigate(from);
          window.location.reload();
        } else {
  ...
  return (
    <div className="auth-container flex flex-col items-center justify-center w-full h-screen bg-gray-100">
      <div className="auth-box bg-white p-8 rounded-md shadow-md w-96">
        ...
        {twoFARequired && (
          <div>
            <label className="block text-gray-700 text-sm font-bold mb-2 mt-4">Enter 2FA Code</label>
            <input
              type="text"
              value={twoFACode}
